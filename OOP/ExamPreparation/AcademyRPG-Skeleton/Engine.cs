    ...
    public class Engine
    {
            ...
            if (currentAsGatherer != null)
            {
                ...
                else
                {
                   // Console.WriteLine("No resource to gather at {0}'s position", current);
                    ...
                    result.AppendLine(string.Format("No resource to gather at {0}'s position", current));
                }
            }
            else
            {
                ...
                result.AppendLine(string.Format("{0} can't do that", current));
            }
        }
        ...
        private void HandleAttackCommand(IControllable current)
        {
            ...
            if (currentAsFighter != null)
            {
                ...
                else
                {
                    ...
                    result.AppendLine(string.Format("No targets to attack at {0}'s position", current));
                }
            }
            else
            {
        ...
        private void HandleGathering(IGatherer gatherer, IResource resource)
        {
            ...
            if (gatheringSuccess)
            {
                ...
                result.AppendLine(string.Format("{0} gathered {1} {2} from {3}", gatherer, resource.Quantity, resource.Type, resource));
        ...
        private void HandleBattle(IFighter attacker, WorldObject defender)
        {
            ...
            result.AppendLine(string.Format("{0} attacked {1} and did {2} damage", attacker, defender, damage));
        }
        ...
        private void HandleGoCommand(string[] commandWords, IControllable current)
        {
            ...
            result.AppendLine(string.Format("{0} is now at position {1}", current, current.Position));
        }
    }
}
